...
public class Main {
	...
	public static void main(String[] args) {
		...
		/*TerminalFunction tf=new GridWorldTerminalFunction(7,7);
		gridWorldDomain.setTf(tf);*/
			...
			case ValueIteration:
				runAlgorithm(analysis, problem, domain, hashingFactory, initialState, new PlannerFactory() {
					...
					public Planner createPlanner(int episodeIndex, SADomain domain, HashableStateFactory hashingFactory, SimulatedEnvironment simulatedEnvironment) {
						return new ValueIteration(domain, 0.99, hashingFactory, 0.0001, episodeIndex);
					}
				}, algorithm);
				break;
			case PolicyIteration:
				runAlgorithm(analysis, problem, domain, hashingFactory, initialState, new PlannerFactory() {
					...
					public Planner createPlanner(int episodeIndex, SADomain domain, HashableStateFactory hashingFactory, SimulatedEnvironment simulatedEnvironment) {
						 ...
						 * iterations).
						 */
						return new PolicyIteration(domain, 0.99, hashingFactory, 0.0001, problem.getNumberOfIterations(Algorithm.PolicyIteration), episodeIndex);
					}
				}, algorithm);
				break;
			default:
	...
	private static Problem createProblem1() {
		...
		numIterationsHashMap.put(Algorithm.ValueIteration, 49);
		numIterationsHashMap.put(Algorithm.PolicyIteration, 30);
		numIterationsHashMap.put(Algorithm.QLearning, 200);
	...
	private static Problem createProblem2() {
		String[] map = new String[] {
				"X0001000100010000011",
				"101110101L1010S11011",
				"10001010100010001011",
				"11101010101111S11011",
				"10001010000010000001",
				"1011101S101010111011",
				"10001010101000100011",
				"10101010101111101011",
				"10100000100010001001",
				"1110101M101010M000G1",
				"10001010001010000011",
				"101110101010101111S1",
				"10001000101000101001",
				"11101010101011101011",
				"10101000101000100011",
				"10101010101L00101111",
				"1000001S000010101001",
				"10101011011010101000",
